...
void Noticias::executar_noticia() {
	...
	if (carta == 0) {
		...
		set_valor(1.500);
		set_texto("Com o cartao de debito da MasterCard você tem muito mais seguranca!");
	}
	else if (carta == 3) {
		set_valor(800);
		...
		set_valor(1.000);
		set_texto("Seu carro foi roubado, mais você tinha seguro auto Itau.");
	}
	else if (carta == 7) {
		set_valor(1.500);
		set_texto("Com o cartao de credito MasterCard você tem muitos beneficios para aproveitar!");
		...
		set_valor(200);
		set_texto("Você acumulou pontos no Km de vantagens e pode trocar por combustivel nos Postos Ipiranga e lanches no AmPm.");
	}
	else if (carta == 12) {
		set_valor(1.000);
		...
		set_valor(500);
		set_texto("Parabens! Você ganhou a promocao do Mundo Nivea e recebeu todos os produtos Nivea na sua casa!");
	}
	else if (carta == 14) {
		set_valor(1.500);
		set_texto("Com o cartao de credito MasterCard você tem muitos beneficios para aproveitar!");
		...
		set_valor(1.500);
		set_texto("O jogo ainda nao tem suporte para o que essa carta queria, então você processou o Julio e ganhou!");
	}
	else if (carta == 18) {
		set_valor(0);
		...
		set_valor(-90);
		set_texto("Chegou a nova linha de produtos Nivea! Você ja esta na fila para comprar!");
	}
	else if (carta == 23) {
		set_valor(-300);
		...
		set_valor(-150);
		set_texto("A sua operadora de celular não tem sinal no local! Para telefonar pague");
	}
	else if (carta == 25) {
		set_valor(-1000);
		...
		set_valor(-250);
		set_texto("Como o seu MasterCard você pode dividir aquele pagamento e pagar agora apenas esta primeira parcela.");
	}
	else if (carta == 27) {
		set_valor(-500);
		...
		set_valor(-200);
		set_texto("Preocupado com o futuro, você colocou dinheiro na sua previdencia Itau.");
	}
	else if (carta == 31) {
		set_valor(-500);
		...
		set_valor(-400);
		set_texto("Voce não usou o cinto de segurança.");
	}
	else if (carta == 35) {
		set_valor(-300);
